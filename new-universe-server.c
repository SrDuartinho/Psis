...
/***
 * @brief Finds the index of a ship in the array based on its character identifier.
 * @param arr Array of ships.
 * @param n Number of ships in the array.
 * @param ch Character identifier of the ship to find.
 * @return Index of the ship if found, -1 otherwise.
 */
    ...
    while(close == 0){
        ...
        while(SDL_PollEvent(&event)){
            if (event.type == SDL_QUIT){
               send_game_state(state_fd, NULL, 0, NULL, 0, NULL, 0); //Notify clients of the game ending
                close = 1;
            }
        }
        ...
        if (strcmp(message_type, "CONNECT") == 0) {
        ...
        } else if (message_type[0] != '\0' && strcmp(message_type, "MOVE") == 0) {    
            ...
            if (pos != -1) {
                ...
                int x = ships[pos].position.x;
                int y = ships[pos].position.y;

                new_position(&x, &y, d);

                ships[pos].position.x = x;
                ships[pos].position.y = y;
        ...
        for (int i = 0; i < n_ships; i++)
            draw_char(rend, font,
                      ships[i].ch,
                      ships[i].position.x,
                      ships[i].position.y, ship_color, ships[i].trash_count);
        ...
        for (int i = 0; i < n_ships; i++){
            for (int j = 0; j < n_trash; j++){
                ...
                float dx = (ships[i].position.x + 20) - trash[j].position.y;
                float dy = (ships[i].position.y + 10) - trash[j].position.x;
        ...
        for (int i = 0; i < n_ships; i++){
            for (int j = 0; j < PLANET_NUM; j++){
                float dx = (ships[i].position.x + 20) - planets[j].y;
                float dy = (ships[i].position.y + 10) - planets[j].x;
                float distance = sqrt(dx * dx + dy * dy);
                if (distance <= 20.0f) {  // within radius of 20
